...
//{$DEFINE pi3}  //comment out for windows 10 compile
 {$DEFINE win10} //comment out for Raspberry pi3 compile

{$ifdef pi3}
uses
 SysUtils, LCLIntf, LCLType, Dialogs, Classes, Controls, Graphics, Forms,
 StdCtrls, ExtCtrls, Menus, Spin, Buttons, math,
 ComCtrls, serial, strutils,
 Messages,
 Clipbrd,
 Registry, PrintersDlgs, Printers, Arrow, LazHelpHTML;
{$else}

uses
 windows,LCLIntf, LCLType, SysUtils, Classes, Controls, graphics,
 StdCtrls, ExtCtrls, Menus, Spin, Buttons,  math,
 Grids, ComCtrls, Arrow, strutils,
Messages,Clipbrd,
Registry, PrintersDlgs, Printers, LazHelpHTML, forms, dialogs;
{$endif}
//uses
//{$IFNDEF FPC}
...
//{$ENDIF}
//  Serial, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
//  StdCtrls, ExtCtrls, Menus, Spin, Buttons, Clipbrd,
//  ComCtrls, Registry, Math, PrintersDlgs, Printers, Arrow, LazHelpHTML;
...
//    MediaPlayer1: TMediaPlayer;
    ...
    procedure paintdib2(Sender: TObject);
...
{$endif}
   ...
   LogName, maindir : string;                  //File name for sript file saves
...
// form1.Memo1.PasteFromClipboard;
   locationxy := form1.memo1.caretpos;
   form1.Memo1.Lines.Insert(locationxy.y, pt);
end;
{$ifdef pi3} 
...
begin
  ...
  if portnum = 1 then FileName := '/dev/ttyUSB0';
  if portnum = 2 then FileName := '/dev/ttyUSB1';
  if portnum = 3 then FileName := '/dev/ttyUSB2';
  if portnum = 4 then FileName := '/dev/ttyUSB3';
  if portnum = 5 then FileName := '/dev/ttyUSB4';
  if portnum = 6 then FileName := '/dev/ttyUSB5';
  if portnum = 7 then FileName := '/dev/ttyUSB6';
  if portnum = 8 then FileName := '/dev/ttyAMA0';
  if portnum = 9 then FileName := '/dev/ttyACM0';
  if portnum = 10 then FileName := '/dev/ttyACM1';
  if portnum = 11 then FileName := '/dev/ttyS0';
  if portnum = 12 then FileName := '/dev/ttyS1';
  if portnum = 13 then FileName := '/dev/ttyS2';
  if portnum = 14 then FileName := '/dev/ttyS3';
  if portnum = 15 then FileName := '/dev/ttyS4';
  if portnum = 16 then FileName := '/dev/ttyS5';
  if portnum = 17 then FileName := '/dev/ttyS6';
  if portnum = 18 then FileName := '/dev/ttyS7';
  if portnum = 19 then FileName := '/dev/ttyS8';
  if portnum = 20 then FileName := '/dev/ttyS9';
...
//  if (comhand > 0) and (comhand < 4294967295) then begin
  ...
  try
   ...
   serial.SerSetParams(comhand,baud,8,NoneParity,1,[]);
   form1.comport1.caption := FileName;
  except
...
/// end;
   ...
   begin
     ...
     if (comhand > 0) and (comhand < 4294967295) then begin
       ...
       a2 := SetUpComm(comhand, 65536, 1);           //Set up for a 32767 byte receive buffer and a 512 byte write buffer
   //    a2 := SetUpComm(comhand, 65536, 1024);           //Set up for a 32767 byte receive buffer and a 512 byte write buffer
       a2 := GetCommState(comhand, d);                //Get the current set up for the port
       dw := d.wreserved;
       dw := d.wreserved1;
       d.BaudRate := baud;                           //Change the speed and bit settings
       d.ByteSize := 8;
       d.Parity := NOPARITY;
       d.StopBits := ONESTOPBIT;
       form1.caption := inttostr(d.flags);
       d.flags := 17;
...
Form1.canvas.draw(0, 0, myimage);
 if myimage.Height <> 513 then myimage.Height := 513;           //Restore the image size to 513x513
 if myimage.Width <> 513 then myimage.Width := 513  ;            //Needed if other images were loaded
 //// removed to prevent black screen on raspberry pi
 //// if myimage.PixelFormat <> pf24bit then myimage.PixelFormat := pf24bit; //Use true RGB for the image
end;
...
////form1.canvas.FillRect(rect(1, 1, 512, 512));
////form1.canvas.Brush.Bitmap := nil;
////myimage.canvas.brush.Canvas.Erase;
...
begin
 ...
 if form1.listbox1.ItemIndex >= 4 then begin
  ...
  m := i3-i2;
  v4 := v4 / maxpoints;
  if abs(b) < 0.000001 then b := 0.000001;
...
var
 i, m, j : integer;
begin
if myimage.PixelFormat <> pf24bit then begin
 myimage.Destroy;
...
procedure checkforbyte; forward;
...
procedure TForm1.paintdib2(Sender: TObject);
begin
  paintdib;
end;
...
begin
  if baud = 460800 then baud := 230400 else baud := 460800;
end;
...
begin
 ...
 while (j = 1) do begin
  ...
  begin
   ...
   resetmodes;
   setgain(p2temp);
   form1.caption := 'N7VK Mini Curve Tracer S/N: ' + inttostr(serialno) + '  ' + inttohex(p2temp,4);
   form1.RunSweep.Enabled := true;
   exit;
  end;
  ...
  if (ic = 256) then j := 0 else begin  //process block of trace sweep data
   ...
   if (form1.on1.checked = true) and (length(form1.edit1.text)< 64) then form1.edit1.text := form1.edit1.text + ctest; //text echo
...
begin
   Try
    Form1.Edit1.Text := 'CalLut_' + inttostr(serialno) + '.dat';
   //
    AssignFile(F, ExtractFilePath(Application.EXEName) + 'callut_' + inttostr(serialno) + '.dat');
    ReSet(F);
    Read(F, x);
    Vcal[0] := x;
    Read(F, Vcal[1]);
...
begin
 ...
 {$ifdef win10}
  form1.Com11.Caption := 'com1';
  form1.Com21.Caption := 'com2';
  form1.Com31.Caption := 'com3';
  form1.Com41.Caption := 'com4';
  form1.Com51.Caption := 'com5';
  form1.Com61.Caption := 'com6';
  form1.Com71.Caption := 'com7';
  form1.Com81.Caption := 'com8';
  form1.Com91.Caption := 'com9';
  form1.Com101.Caption := 'com10';
  form1.Com111.Caption := 'com11';
  form1.Com121.Caption := 'com12';
  form1.Com131.Caption := 'com13';
  form1.Com141.Caption := 'com14';
  form1.Com151.Caption := 'com15';
  form1.Com161.Caption := 'com16';
  form1.Com171.Caption := 'com17';
  form1.Com181.Caption := 'com18';
  form1.Com191.Caption := 'com19';
  form1.Com201.Caption := 'com20';
 ...
 if cstats <> 0 then begin
   ...
   transmitcombyte(ord('0'));
   transmitcombyte(13);
   sleep(80);                //wait for return of data
...
i := Pos('setbasev ', s);
 ...
 //  myimage.SaveToFile(ChangeFileExt(s2,'.bmp'));
   with jp do begin
    ...
    //Compress;
    assign(myimage);
    SaveToFile(ChangeFileExt(s2,'.jpg'));
   end;
  finally
    jp.free;
  end;
 ...
 if i = 1 then begin
   ...
   es := 'File Not Found';
   try
    MyImage.LoadFromFile(maindir+sc);
    PaintDib;
   finally
   end;
   exit;
...
begin
  ...
  CloseFile(F);
  maindir := ExtractFilePath(form1.OpenDialog1.filename);
 end; 
end;
...
var
 ...
 v1, i1 : single;
begin
 if form1.SamplePoint.Value < 0 then form1.SamplePoint.Value := 255;
 if form1.SamplePoint.Value > 255 then form1.SamplePoint.Value := 0;
 i := hash(0,form1.TraceNumber.value,form1.SweepStep.value,0);  //find a table entry
...
//  if form1.RadioSR.ItemIndex <> 1 then formP.Label1.Caption :=  'Set R to 1.5K position, run again';
  doReadVoltages(256);
  sleep(delay2);
  checkforbyte;
  calnum := 1;
 ...
 if radiocal.ItemIndex = 1 then begin  //Collector Voltage Measure Cal
  if form1.RadioSR.ItemIndex > 1 then formP.Label1.Caption :=  'R not at 1.5K position, run again';
  doReadVoltages(256);
  sleep(delay2);
  checkforbyte;
 ...
 if radiocal.ItemIndex = 6 then begin  //Base DC set Voltage Zero
  if (form1.RadioSR.ItemIndex <> 3) or (form1.SetBaseV.Enabled = false) then formP.Label1.Caption :=  'R not at  100K position or switch at BV, run again';
  CollectorV.Value := 0;
  doSetCollector;
  sleep(15);
  BaseVolts.Value := 0;
 ...
 if radiocal.ItemIndex = 7 then begin  //Base DC set Voltage Cal
  if (form1.RadioSR.ItemIndex <> 3) or (form1.SetBaseV.Enabled = false) then formP.Label1.Caption :=  'R not at 100K position or switch not at BV, run again';
  CollectorV.Value := 0;
  doSetCollector;
  sleep(15);
  BaseVolts.Value := 900;
 ...
 if radiocal.ItemIndex = 8 then begin  //Base DC set Current Zero
  if (form1.RadioSR.ItemIndex <> 3) or (form1.SetBaseV.Enabled = true) then formP.Label1.Caption :=  'R not at 100K position or switch not at BI, run again';
  CollectorV.Value := 0;
  doSetCollector;
  sleep(15);
  Baseua.Value := 0;
 ...
 if radiocal.ItemIndex = 9 then begin  //Base DC set Current Cal
  if (form1.RadioSR.ItemIndex <> 2) or (form1.SetBaseV.Enabled = true) then formP.Label1.Caption :=  'R not at 15K position or switch not at BI, run again';
  CollectorV.Value := 0;
  doSetCollector;
  sleep(15);
  Baseua.Value := 1900;
...
begin
 ...
 if opencomport(1) <> 0 then begin
  ...
  {$else}
  form1.comport1.caption := 'ttyUSB0';
  {$endif}
  form1.RunSweep.enabled := true;
  dostartload;
 end else
...
begin
 ...
 if opencomport(2) <> 0 then begin
  ...
  {$else}
  form1.comport1.caption := 'ttyUSB1';
  {$endif}
  form1.RunSweep.enabled := true;
  dostartload;
 end else
...
begin
 ...
 if opencomport(3) <> 0 then begin
  ...
  {$else}
  form1.comport1.caption := 'ttyUSB2';
  {$endif}
    form1.RunSweep.enabled := true;
    dostartload;
 end else
...
begin
 ...
 if opencomport(5) <> 0 then begin
  ...
  {$else}
  form1.comport1.caption := 'ttyUSB4';
  {$endif}
    form1.RunSweep.enabled := true;
    dostartload;
 end else
...
begin
 ...
 if opencomport(6) <> 0 then begin
  ...
  {$else}
  form1.comport1.caption := 'ttyUSB5';
  {$endif}
    form1.RunSweep.enabled := true;
    dostartload;
 end else
...
begin
 ...
 if opencomport(7) <> 0 then begin
  ...
  {$else}
  form1.comport1.caption := 'ttyUSB6';
  {$endif}
    form1.RunSweep.enabled := true;
    dostartload;
 end else
...
begin
 ...
 if opencomport(8) <> 0 then begin
  ...
  {$else}
  form1.comport1.caption := 'ttyAMA0';
  {$endif}
    form1.RunSweep.enabled := true;
    dostartload;
 end else
...
begin
 ...
 if opencomport(9) <> 0 then begin
  ...
  {$else}
  form1.comport1.caption := 'ttyACM0';
  {$endif}
    form1.RunSweep.enabled := true;
    dostartload;
 end else
...
begin
 ...
 if opencomport(10) <> 0 then begin
  ...
  {$else}
  form1.comport1.caption := 'ttyACM1';
  {$endif}
    form1.RunSweep.enabled := true;
    dostartload;
 end else
...
begin
// if  (OpenDocument('HTML_Help\Mini_CT_help.html'))  then begin
  OpenDialog1.DefaultExt := '*.html';
  OpenDialog1.FilterIndex := 3;
  OpenDialog1.Filename := './HTML_Help/Mini_CT_help.html';
  if OpenDialog1.Execute then begin    //Open the file dialog and let the user pick a file name
...
begin
 ...
 if opencomport(18) <> 0 then begin
  ...
  {$ifdef win10}
  form1.comport1.caption := 'com18';
  {$else}
  form1.comport1.caption := 'ttyS7';
  {$endif}
    form1.RunSweep.enabled := true;
    dostartload;
 end else
...
begin
 ...
 if opencomport(19) <> 0 then begin
  ...
  {$ifdef win10}
  form1.comport1.caption := 'com19';
  {$else}
  form1.comport1.caption := 'ttyS8';
  {$endif}
    form1.RunSweep.enabled := true;
    dostartload;
 end else
...
begin
 ...
 if opencomport(20) <> 0 then begin
  ...
  {$ifdef win10}
  form1.comport1.caption := 'com20';
  {$else}
  form1.comport1.caption := 'ttyS9';
  {$endif}
    form1.RunSweep.enabled := true;
    dostartload;
 end else
